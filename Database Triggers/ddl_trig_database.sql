...
31-Mar-2021	Srimathi	Added Delete_Flag in intermediate insert statement
*/
DECLARE @id int;
DECLARE @obj_ID int;
...
DECLARE @sandboxSchema varchar(100);
	...
	SELECT @id  = USERID, @sandboxSchema = Field1 FROM APPADMIN.TI_ADM_USER_LU WHERE USEREMAIL=trim(EVENTDATA().value('(/EVENT_INSTANCE/LoginName)[1]','varchar(max)'))
	...
	BEGIN
	     ...
	     Insert into [AppAdmin].[ti_adm_ObjectOwner_Intermediate](ObjectName, ObjectType, SchemaName, CreatedDate,  CreatedBy,UserEmail,IsActive,Delete_Flag)
		 Values ( EVENTDATA().value('(/EVENT_INSTANCE/ObjectName)[1]','nvarchar(max)') , 'Table', EVENTDATA().value('(/EVENT_INSTANCE/SchemaName)[1]','nvarchar(max)'), EVENTDATA().value('(/EVENT_INSTANCE/PostTime)[1]','datetime'),  @id,trim(EVENTDATA().value('(/EVENT_INSTANCE/LoginName)[1]','varchar(max)')),1,0)
