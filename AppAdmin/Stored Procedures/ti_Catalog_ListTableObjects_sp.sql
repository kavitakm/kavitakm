--sp_helptext '[AppAdmin].[ti_Catalog_ListTableObjects_sp]'
...
** Date      : 23-Aug-2019                
 ...
 Dinesh 15-02-2021   Added AnalyticsPBIDataSet in output and relevent join condition.    
*******************************************************************************/                
...
--print @userid;               
  ...
  ,VIDataSet.Object_GUID as AnalyticsPBIDataSet  
  FROM          
 ...
 SELECT               
  ISnull(OBJ.ObjectID,0) AS [SelectedObjectID]      
 ...
 or    
 obj.createdby = @userid)     
 and (Len(@SearchText) > 0   and  OBJ.SchemaName = @SchemaName and ( OBJ.ObjectName  =  @SearchText  ) and obj.isactive = 1 and OBJ.objectType ='Table')  
