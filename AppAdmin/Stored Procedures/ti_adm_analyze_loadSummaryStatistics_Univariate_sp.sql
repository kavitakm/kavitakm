...
SET @ObjectID=(SELECT objectID FROM appAdmin.ti_adm_objectowner            
   WHERE objectname=@tablename and SchemaName=@schemaName and objectType='Table' and isActive=1)          
...
IF NOT EXISTS(SELECT * FROM AppAdmin.ti_adm_SummaryStatistics a, #ti_adm_SummaryStatistics tmp         
     WHERE ISNULL(a.ObjectID,'')=ISNULL(tmp.ObjectID,'') and ISNULL(a.Column1Name,'') = ISNULL(tmp.Column1Name,'')        
    )          
BEGIN          
IF OBJECT_ID('tempdb.dbo.#Temp') Is Not Null          
 DROP TABLE #Temp          
 --get the datatype for the input column name         
SELECT @Col_DT = DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = @SchemaName AND TABLE_NAME = @tableName           
...
--For the numerical datatype we populate all mean,median,mode etc ..        
 ...
 INSERT INTO #TempMode EXEC [AppAdmin].[ti_analyze_Stats_Mode_sp] @SchemaName, @TableName, @ColumnName, ''          
 ...
 CREATE TABLE #TempPercentile(P50 Decimal(20,2)          
     ,P25 Decimal(20,2)          
     ,P75 Decimal(20,2)          
     )          
 ...
 UPDATE #ti_adm_SummaryStatistics           
 SET Median = tmp.P50, P50=tmp.P50, P25=tmp.P25, P75=tmp.p75          
 FROM (SELECT * FROM #TempPercentile) tmp          
        
END          
          
INSERT INTO AppAdmin.ti_adm_SummaryStatistics(ObjectID,Column1Name,Column1Value,Column2Name,Column2Value,Column3Name,Column3Value,Column4Name,Column4Value      
...
SELECT * FROM AppAdmin.ti_adm_SummaryStatistics WHERE ObjectID = @ObjectID   and Column1Name=@ColumnName and Column2Name IS NULL       
