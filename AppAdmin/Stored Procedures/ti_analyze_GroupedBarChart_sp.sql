...
--exec [AppAdmin].[ti_analyze_GroupedBarChart_sp] 'Sandbox','passenger','[AlertFlag]','[Date]'
...
--Tablename and column name to enclose with square brackets
	...
	ELSE IF @col1_dt IN ('Money','Int','TinyInt','bigint','smallint','bit','numeric','small money','float','real','decimal')   
		SET @Replace_Null_With1 = 'ISNULL('  + @column1Name + ',''0'')'
	ELSE
	...
	ELSE IF @col2_dt IN ('Money','Int','TinyInt','bigint','smallint','bit','numeric','small money','float','real','decimal')   
		SET @Replace_Null_With2 = 'ISNULL('  + @column2Name + ',''0'')'
	ELSE
...
--SET @query = 'SELECT @cols = STUFF((SELECT DISTINCT '','' + QUOTENAME(' + @Replace_Null_With2 + ') FROM '  + @schemaname + '.'  + @tablename + ' FOR XML PATH(''''), TYPE).value(''.'', ''NVARCHAR(MAX)''),1,1,'''')';
	...
	SET @query = ' SELECT * FROM   (  SELECT ' + @Replace_Null_With1 + ' ' + @column1Name + ', ' + @Replace_Null_With2 + ' ' + @column2Name + ' FROM ' + @schemaname + '.'  + @tablename + ' ) t PIVOT( COUNT( ' + @column2Name +')  FOR ' + @column2Name +' IN (' + @cols + ' )) AS pivot_table order by 1,2;'
	  --Print @query
	execute(@query)
